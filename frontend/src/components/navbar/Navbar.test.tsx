...
vi.mock("../../features/cart/hooks/useCartSummary", () => ({
    useCartSummary: vi.fn(),
}));

vi.mock("./NavSidebar", () => ({
    NavSidebar: (props: unknown) => {
        navSidebarMock(props);
        return <div>NavSidebar</div>;
    },
}));
...
describe("Navbar", () => {
    beforeEach(() => {
        ...
        (useCartSummary as any).mockReturnValue({
            totalItems: 0,
        });
    ...
    it("opens cart panel", () => {
        ...
        fireEvent.click(
            screen.getByRole("button", { name: "Cart: 5" })
        );
    ...
    it("passes sidebar open state to NavSidebar", () => {
        ...
        const props = navSidebarMock.mock.calls[0][0];

        expect(props.isSidebarOpen).toBe(true);
        expect(isOpen).toHaveBeenCalledWith("sidebar");
    });
    ...
    it("passes search open state to SearchBar", () => {
        ...
        const props = searchBarMock.mock.calls[0][0];

        expect(props.isOpen).toBe(true);
        expect(isOpen).toHaveBeenCalledWith("search");
    });
    ...
    it("passes cart open state to Cart", () => {
        ...
        const props = cartMock.mock.calls[0][0];

        expect(props.isOpen).toBe(true);
        expect(isOpen).toHaveBeenCalledWith("cart");
    });
